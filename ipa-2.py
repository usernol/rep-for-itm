...
def caesar_cipher(message, shift):
    ...
    for letter in message:
        if letter == " ":
            ...
            cipher_message += chr((ord(letter)+ shift - 65) % 26 + 65)
...
def shift_by_letter(letter, letter_shift):
    ...
    shift_letter = (letter_index + shift_index) % 26
...
def vigenere_cipher(message, key):
    ...
    long_key = key * (len(message) // len(key)) + key[:len(message) % len(key)]

    for i in range(len(message)):
        cipher_letter = message[i]
        cipher_key = long_key[i]

        if cipher_letter == " ":
            output_message += " "
        else:
            letter_index = alphabet.index(cipher_letter)
            key_index = alphabet.index(cipher_key)

            shifted_index = (letter_index + key_index) % 26
            shifted_letter = alphabet[shifted_index]
            output_message += shifted_letter
